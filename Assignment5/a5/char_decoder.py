...
class CharDecoder(nn.Module):
    ...
    def forward(self, input, dec_hidden=None):
        ...
        dec_hiddens, dec_hidden = self.charDecoder(x_t, dec_hidden)
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        (length, batch_size, e_char) = scores.shape
        cross_entropy_loss = loss(scores.contiguous().view(-1, e_char),
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        current_char = torch.full((1, batch_size), start_token, dtype=torch.long, device=device)
        dic_hidden = (initialStates[0], initialStates[1])
        output = list('' for x in range(batch_size))
        unfinished = list(x for x in range(batch_size))
        for step in range(max_length+1):
            scores, dic_hidden = self.forward(current_char, dic_hidden)
            current_char = scores.argmax(dim=-1)
            for i in unfinished:
                if current_char[0][i] != end_token:
                    output[i] += self.target_vocab.id2char[current_char[0][i].item()]
                else:
                    unfinished.remove(i)
