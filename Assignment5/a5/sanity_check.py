...
def question_1g_sanity_check(conv):
    """
    Sanity check for highway() function.
    """
    print("-" * 80)
    print("Running Sanity Check for conv1d")
    print("-" * 80)

    # 测试数据为batch_size*e_word
    output_expect_size = [BATCH_SIZE, EMBED_SIZE]
    # 测试数据
    test_data = torch.zeros(BATCH_SIZE, 50, 20)
    output = conv(test_data)

    assert list(output.size()) == output_expect_size, "output size should be {}\n but is {}".format(output_expect_size,
                                                                                                    list(output.size()))
...
def question_1f_sanity_check(highway):
    ...
    """
    print("-" * 80)
    print("Running Sanity Check for highway network")
    print("-" * 80)

    # 测试数据为batch_size*e_word
    output_expect_size = [BATCH_SIZE, EMBED_SIZE]
    # 测试数据
    test_data = torch.zeros(BATCH_SIZE, EMBED_SIZE)
    output = highway(test_data)
...
def main():
    ...
    conv = CNN(e_char=50, kernel_size=KERNEL_SIZE,e_word=EMBED_SIZE)
