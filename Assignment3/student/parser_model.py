...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = torch.empty((self.embed_size * self.n_features, self.hidden_size))
        ...
        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform_(self.embed_to_hidden_bias))

        self.dropout = torch.nn.Dropout(p = dropout_prob)

        self.hidden_to_logits_weight = torch.empty((self.hidden_size, self.n_classes))
        self.hidden_to_logits_weight = nn.Parameter(nn.init.xavier_uniform_(self.hidden_to_logits_weight))

        self.hidden_to_logits_bias = torch.empty((self.n_classes,))
        self.hidden_to_logits_bias = nn.Parameter(nn.init.uniform_(self.hidden_to_logits_bias))
    ...
    def embedding_lookup(self, w):
        ...
        index = torch.Tensor.view(w, (-1, ))
        x = torch.index_select(self.embeddings, 0, index)
        x = torch.Tensor.view(x, (batch_size, -1))
    ...
    def forward(self, w):
        ...
        h = torch.nn.functional.relu(x.matmul(self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
